...
initialNbrOfAgentsVector = linspace(100,2000, 10);
...
diffusionRate = 0.4;
hungerRate = 0.01;
collectionGrowthRatio = 0.1*ones(gridLength);
...
landscape = ones(gridLength) * 0.5 + rand(gridLength)*landscapeNoise;
growthRate = InitializeGrid(gridLength, percentHigherResources, maxGrowthRate, diffusionSteps, k);
%collectionRate = collectionGrowthRatio * growthRate;
collectionRate = collectionGrowthRatio;
...
fprintf('Test %d of %d completed after %.2f seconds', iTest, nbrOfTests, toc(elapsedTime));
