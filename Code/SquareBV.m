function CellArea=SquareBV(xPos,yPos,toggleplot,varargin)
...
positions = [xPos yPos];
[uniquePositions,~,iUniquePositions] = unique(positions,'rows');
x = uniquePositions(:,1);
y = uniquePositions(:,2);
...
for index = 1 : LenC
    ...
    if all(NormVSet)
        ...
        %%%%%%%%%% Written by Gabriella %%%%%%%%%%
        numberOfAgentsSharingThisPosition = nnz(iUniquePositions == index);
        CellArea(index) = CellArea(index)/numberOfAgentsSharingThisPosition;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
